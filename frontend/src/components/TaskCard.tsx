...
export default function TaskCard({ taskId, employer, worker, amount, status }: TaskCardProps) {
    ...
    const handleSubmitWork = async () => {
        if (!evidenceHash) {
            alert("Lütfen bir IPFS Hash veya kanıt linki girin!");
            return;
        }
        setLoading(true);
        try {
            ...
            await submitTask(taskId, evidenceHash, address);
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mt-4 shadow-md w-full">
            ...
            {status === 0 && (
                <div className="flex flex-col gap-3 border-t border-gray-700 pt-4">
                    <input
                        type="text"
                        placeholder="IPFS CID veya Github Linki..."
                        value={evidenceHash}
